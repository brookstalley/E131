...
{
  if (memcmp(packet->acn_id, ACN_ID, sizeof(packet->acn_id)) != 0) return ERROR_ACN_ID;

  if (htonl(packet->root_vector) != VECTOR_ROOT) return ERROR_VECTOR_ROOT;

  if (htonl(packet->frame_vector) != VECTOR_FRAME) return ERROR_VECTOR_FRAME;

  if (packet->dmp_vector != VECTOR_DMP) return ERROR_VECTOR_DMP;
  return ERROR_NONE;
}
...
void E131::dumpError(e131_error_t error)
{
  switch (error) {
    case ERROR_ACN_ID:
      ...
      for (uint8_t i = 0; i < sizeof(ACN_ID); i++) Serial.print(packet->acn_id[i], HEX);
      Serial.println("");
      break;
